...
test_that("Constructor works", {
  rate <- InterestRate(c(0.03, 0.04, 0.05), c(0, Inf, 0), "act/365")
  expect_equal(field(rate, "value"), c(0.03, 0.04, 0.05))
  expect_equal(field(rate, "compounding"), c(0, Inf, 0))
  expect_equal(field(rate, "day_basis"), c("act/365", "act/365", "act/365"))
  rate <- InterestRate(0.03, c(0, Inf, 0), c("act/365", "act/360", "30e/360"))
  expect_equal(field(rate, "value"), rep(0.03, 3))
  expect_equal(field(rate, "compounding"), c(0, Inf, 0))
  expect_equal(field(rate, "day_basis"), c("act/365", "act/360", "30e/360"))
})
...
test_that("convert method works", {
  ...
  expect_equal(field(rate_new, "value"), 0.0364191596790165, tolerance = 1e20)
  rate_new <- as_InterestRate(rate, 4, "act/360")
  expect_equal(field(rate_new, "value"), 0.0365854583997862, tolerance = 1e20)
  rate_new <- as_InterestRate(rate, 4, "act/365")
  expect_equal(field(rate_new, "value"), 0.0370959387671626, tolerance = 1e20)
})
...
test_that("to_rate method works", {
  ...
  expect_equal(
    as.double(as_InterestRate(df, 2, "act/365")), 0.0102793669522563,
    tolerance = 1e20
  )
  df <- DiscountFactor(0.95, ymd("20100101"), ymd("20150101"))
  expect_equal(
    as.double(as_InterestRate(df, Inf, "act/365")), 0.010253040772977,
    tolerance = 1e20
  )
  df <- DiscountFactor(0.95, ymd("20100101"), ymd("20100330"))
  expect_equal(
    as.double(as_InterestRate(df, -1, "30/360us")), 0.202247191011236,
    tolerance = 1e20
  )
})
...
test_that("Other discount factor methods work", {
  expect_equal(
    as.numeric(DiscountFactor(0.95, ymd("20100101"), ymd("20150101"))),
    0.95
  )
})
